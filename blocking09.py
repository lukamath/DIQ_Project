...
# raw and uncleaned dataset
file_path = "movies.csv"
# I read it as test file to process it line by line 
with open(file_path, "r", encoding="utf-8") as file:
    input_text = file.read()

# regex pattern as result of dataset analysis to discover recurrent patterns to use to build a valid csv
pattern = r'(?:^(?!.*\)).*,";;;;;|^.*,,";;;;;$|M";;;;;|",,,|^".*[0-9].*,+$|^".*\$.*M$)$'

#--------------------------------------------------------------------------
#I use re.split, but I need to save the matching line

# Find all matching lines using re.finditer()
matching_lines = [match.group() for match in re.finditer(pattern, input_text, flags=re.MULTILINE)]
matching_lines.append("The End") #I use this to align the dimensions of matching_lines with movie_blocks that has an empty block in the end
print(f"lenght of matching lines:  {len(matching_lines)}")
movie_blocks = re.split(pattern, input_text, flags=re.MULTILINE)
print(f"lenght of movie_blocks:  {len(movie_blocks)}")

#for each block I attach the matching line as last
for i in range(0, len(movie_blocks)):
    if movie_blocks[i].strip():
        movie_blocks[i]=movie_blocks[i]+matching_lines[i]
...
print(f"All blocks saved to: {output_file_path} - # of blocks: {i}")
...
norm3_file_path="norm3b_file.csv"
target_sequence_4='            ""'
target_sequence_5=', '
target_sequence_6='"'
